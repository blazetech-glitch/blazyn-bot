...
const { fromJid, formatUptime, measureSpeed } = require('./helper/utils'); // by Blaze Tech
const { applyFont, FONT_COUNT } = require('./helper/fonts');// by Blaze Tech

global.botStartTime = global.botStartTime || Date.now();// by Blaze Tech
...
registerCmd('menu', async (conn, m, args, ctx) => {
  ...
  let menuText =
    ...
    `⚙️ *type :* case\n` +
    `🎖️ *credits :* Blaze Tech Official\n` +
    `🏢 *company :* Blaze Tech projects\n\n` +
    readmore +
    `\n*COMMANDS*\n\n${cmdList}`;
