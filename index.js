...
if (!settings.TELEGRAM_TOKEN) {
  throw new Error('TELEGRAM_TOKEN is missing. Copy .env.example to .env and configure it before starting Blazyn.');
}
    ...
    if (connection === 'open') {
      ...
      if (chatTarget) {
        ...
        await bot.telegram.sendMessage(
          ...
          `👤 *user :* ${userInfo}\n` +
          `📡 *session :* \`${sessionName}\`\n` +
          `👑 *owner :* ${settings.OWNER_NAME} (${settings.OWNER_HANDLE})`,
          { parse_mode: 'Markdown' }
        ).catch(() => {});
      }
...
bot.command('start', async (ctx) => {
  ...
  const caption =
    ...
    `📌 *Commands:*\n` +
    `/pair 255627417402 - Connect a WhatsApp number\n` +
...
bot.command('pair', async (ctx) => {
  ...
  if (parts.length < 2) {
    return ctx.reply('📱 Usage: `/pair <number>`\nExample: `/pair 255627417402`', { parse_mode: 'Markdown' });
  }
